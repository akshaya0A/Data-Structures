...
public class BinarySearchTree<K extends Comparable<K>, V> implements OrderedDeletelessDictionary<K, V> {
    ...
    public K findNextKey(K key) {
        return findNextKey(key, root);
    }

    // recursive helper
    private K findNextKey(K key, TreeNode<K, V> curr) {
        if (curr == null) {
            return null;
        }
        K next = null;
        while (curr != null) {
            if (curr.key.compareTo(key) <= 0) {
                // Current key is <= to the target, so move right to find a smaller key
                curr = curr.right;
            } else {
                // Current key works so try another one to see if we can get closer to the next
                // key
                next = curr.key;
                curr = curr.left;
            }
    ...
    public K findPrevKey(K key) {
        return findPrevKey(key, root);
    }

    // recursive helper
    private K findPrevKey(K key, TreeNode<K, V> curr) {
        if (curr == null) {
            return null;
        }
        K prev = null;
        while (curr != null) {
            if (curr.key.compareTo(key) >= 0) {
                // Current key is >= to the target, so move left to find a smaller key
                curr = curr.left;
            } else {
                // Current key works so try another one to see if we can get closer to the prev
                // key
                prev = curr.key;
                curr = curr.right;
            }
